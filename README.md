...
**Ускоренная подготовка QR (0.6.2):** параллельная генерация и показ после первой
готовой порции. Локально четыре процесса дали около 3× ускорения.
[Команда для VDI](docs/usage/fast-generation.md) · [Измерения](docs/reports/qr-generation.md).
...
## Большие файлы: порционная загрузка

Новый `index.html` подгружает матрицы небольшими порциями с ограниченным кэшем.
Repeat поддерживает архив до 64 МиБ; LT сохраняет предел 16 МиБ. В плеере видны
оценка полного цикла передачи и остаток цикла, в том числе в fullscreen.
[Обновление существующего плеера и тест 10/32 МиБ на VDI](docs/usage/paged-vdi-test.md).
