...
Статус: **И00 в работе: инструменты и локальные проверки готовы; реальный JupyterHub/VDI ещё проверяется**. Дата: 15 сентября 2026 года.
...
И00 имеет статус **в работе** (оставшиеся замеры VDI); И01 **реализована и проверена локально** на Python 3.10/3.12; И02 **реализована и проверена локально**, приёмка в JupyterHub открыта; И03–И09 и Э01 — **запланировано**. Зависимости означают требуемые результаты, а не календарное расписание. Результаты И00: [отчёт о среде](docs/reports/environment.md), запуск: [инструкция](docs/usage/iteration-00.md).
