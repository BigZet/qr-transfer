...
Статус: **И01–И06 реализованы локально; три приёма малого каталога на хосте успешны, приёмка Full HD/1 МиБ открыта**. Дата: 16 сентября 2026 года.

Основание: [требования и архитектура](PROJECT.md), [исследование технологий](RESEARCH.md), текущие `code.py` и `qr.py`. Этот документ задаёт порядок работ; подробные задачи и проверки находятся в документах итераций. Предлагаемые модули и команды последующих итераций — проектируемые интерфейсы, а не уже реализованный API.
...
И00 имеет статус **в работе** (оставшиеся замеры VDI); И01 **реализована и проверена локально** на Python 3.10/3.12; И02 **реализована и проверена локально**, приёмка в JupyterHub открыта; И03 **реализована и проверена локально** ([отчёт](docs/reports/browser-mono.md)); После проверки пользователя подтверждены три успешных приёма и распаковки малого каталога на хосте; критерий Full HD/1 МиБ открыт. И04 **реализована и проверена локально** ([отчёт](docs/reports/receiver-multiqr.md)), сравнение на VDI открыто. И05 **реализована и проверена локально** ([отчёт](docs/reports/fec.md)); малый LT-архив проверен по SHA-256, три распакованных файла совпали с И03. Последний resume перезаписал исходные метрики; сравнительные замеры И04/И05 открыты. И06 **реализована экспериментально и проверена локально** ([отчёт](docs/reports/rgb-qr.md)); цвет на VDI/Э01 не измерен. И07–И09 и Э01 — **запланировано**. Зависимости означают требуемые результаты, а не календарное расписание. Результаты И00: [отчёт о среде](docs/reports/environment.md), запуск: [инструкция](docs/usage/iteration-00.md).
...
Необязательные кодеки устанавливаются отдельно: отсутствие cimbar/FEC не должно мешать моно QR. Имена групп зависимостей определить в И01: общее ядро, sender, receiver, дополнительные кодеки, инструменты разработки. Пользователь подтвердил текущий Windows-хост как приёмник и доступность терминала JupyterHub. Точная ОС JupyterHub ещё определяется; предполагаемая Ubuntu не считается подтверждённой.
...
## Дополнение И06: большие наборы матриц

В версии 0.6.1 реализованы порционная загрузка index, ограниченный кэш и видимая
оценка времени цикла. Repeat через порционный плеер допускает архив 64 МиБ;
лимит LT 16 МиБ сохраняется. [Локальный отчёт](docs/reports/paged-player.md) ·
[Подготовленный тест 10/32 МиБ на VDI](docs/usage/paged-vdi-test.md).

В 0.6.2 добавлены ограниченный пул генерации QR и `--live`: публикация плеера
после первой порции, ожидание будущих порций без пропуска кадров. Локально Segno
с четырьмя процессами ускорился в 2,98 раза; live repeat/LT восстановили файлы
в точности. 96 тестов прошли на Python 3.10/3.12. Это дополнение И06;
И07 остаётся открытой. В тесте пользователя 2026-09-16 подтверждён один полный
RGB8/repeat прогон 10 МиБ при захвате 4K: 648,969 с, 15,78 КиБ/с, точное
восстановление. Пользователь подтвердил /proxy/, интервал 300 мс и два цикла.
[Отчёт VDI](docs/reports/vdi-paged.md). Ранний старт и скорость
подготовки на VDI ещё требуют отчёта отправителя; крупный LT завершился ошибкой.
[Отчёт](docs/reports/qr-generation.md) · [Запуск на VDI](docs/usage/fast-generation.md).

Исследование receive-fast-3: копия LT-журнала успешно восстановлена и дополнена
до точного архива (119/119 блоков). Причина падения приёмника не воспроизведена;
добавлена диагностика этапа и цепочки исключений.
[Повторная проверка LT](docs/usage/lt-recheck.md).
