...
Статус: **И01–И04 реализованы локально; три приёма малого каталога на хосте успешны, приёмка Full HD/1 МиБ открыта**. Дата: 16 сентября 2026 года.
...
И00 имеет статус **в работе** (оставшиеся замеры VDI); И01 **реализована и проверена локально** на Python 3.10/3.12; И02 **реализована и проверена локально**, приёмка в JupyterHub открыта; И03 **реализована и проверена локально** ([отчёт](docs/reports/browser-mono.md)); После проверки пользователя подтверждены три успешных приёма и распаковки малого каталога на хосте; критерий Full HD/1 МиБ открыт. И04 **реализована и проверена локально** ([отчёт](docs/reports/receiver-multiqr.md)), сравнение на VDI открыто. И05–И09 и Э01 — **запланировано**. Зависимости означают требуемые результаты, а не календарное расписание. Результаты И00: [отчёт о среде](docs/reports/environment.md), запуск: [инструкция](docs/usage/iteration-00.md).
